...
def extract_distribution_params(model: AbstractModel):
    ...
    def extract_params(distribution):
        ...
        elif isinstance(distribution, dist.LeftTruncatedDistribution):
            return {
                'class': dist.TruncatedNormal,
                'loc': distribution.base_dist.loc,
                'scale': distribution.base_dist.scale,
                'low': distribution.low,
                'base_dist': distribution.base_dist
            }
        else:
            return {
                "class": distribution.__class__,
                **distribution.__getstate__(),
            }

    return {
        key: extract_params(value)
